...
<head>
    ...
    <style>
        ...
        body {
            /* On garde ton background original avec l'overlay sombre pour la lisibilité */
        ...
        /* --- GLASSMORPHISM BASE (Tes réglages originaux) --- */
        .glass-section {
        ...
        /* --- BOUTONS CORAL (Forcé comme tu l'avais demandé) --- */
        .btn-coral {
        ...
        /* --- CARROUSEL & SCREENSHOTS (Optimisé pour ton Asus TUF) --- */
        .carousel-container {
            display: flex;
            overflow-x: auto;
...
<body class="antialiased text-white font-sans" x-data="{
    openAuth: {{ request()->query('auth_trigger') === 'login' || $errors->any() ? 'true' : 'false' }},
    authMode: '{{ $errors->has('name') || old('role') ? 'signup' : (request()->query('auth_trigger') === 'login' ? 'login' : 'login') }}',
    ...
    <nav
        ...
        <a href="/" class="flex items-center gap-2 md:gap-3 group">
            <div
                class="w-7 h-7 md:w-8 md:h-8 btn-coral rounded-lg flex items-center justify-center font-black italic shadow-lg shadow-coral/30 text-sm md:text-base transition-transform group-hover:scale-110">
                V</div>
                    ...
                    <button @click="profileDropdown = !profileDropdown"
                        ...
                        <div class="relative">
                            <div
                                ...
                                <span
                                    class="text-[11px] font-black text-coral">{{ strtoupper(substr(Auth::user()->name, 0, 2)) }}</span>
                            </div>
                            <div
                                class="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-[#1a1a1a] rounded-full">
                            </div>
                        </div>
                        <div class="hidden md:flex flex-col text-left">
                            <span
                                class="text-[11px] font-black uppercase tracking-wider text-white leading-none">{{ Auth::user()->name }}</span>
                            <span
                                class="text-[9px] text-gray-500 uppercase tracking-tighter mt-1">{{ Auth::user()->role === 'restaurateur' ? 'Chef Account' : 'Diner Account' }}</span>
                        </div>
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-600 transition-transform"
                            :class="profileDropdown ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                        ...
                        <div class="p-3 border-b border-white/5">
                            ...
                            @if (Auth::user()->role === 'restaurateur')
                                <a href="{{ route('vendor.dashboard') }}"
                                    ...
                                    <span class="text-xs font-bold text-white group-hover:text-vidish-coral">Vendor
                                        Dashboard</span>
                                </a>
                            @else
                                <a href="{{ route('video.feed') }}"
                                    ...
                                    <span class="text-xs font-bold text-white group-hover:text-vidish-coral">Go to
                                        Feed</span>
                                </a>
                            @endif
                        </div>
    ...
    <main class="pb-24 px-6 flex flex-col items-center gap-16">
        ...
        <section class="w-full max-w-6xl px-4">
            <div class="glass-section rounded-[40px] overflow-hidden shadow-2xl">
                    ...
                    <table class="w-full glass-table border-collapse">
                        <thead>
                            <tr class="bg-white/[0.02]">
                                <th
                                    class="text-left py-6 px-8 text-coral font-black uppercase tracking-widest text-[10px]">
                                    Features</th>
                                <th
                                    class="text-center py-6 px-8 text-coral font-black uppercase tracking-widest text-[10px]">
                                    Diner</th>
                                <th
                                    class="text-center py-6 px-8 text-coral font-black uppercase tracking-widest text-[10px]">
                                    Chef</th>
                ...
                <div class="p-6 bg-white/[0.02] text-center">
                    <p class="text-[9px] text-gray-600 uppercase tracking-[0.3em]">Built for the future of food
                        discovery</p>
                </div>
            </div>
        </section>
        <section class="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
            @unless (Auth::check() && Auth::user()->role === 'restaurateur')
                <div
                    ...
                    @auth
                        <a href="{{ route('video.feed') }}"
                            class="btn-coral w-full py-4 md:py-5 rounded-xl md:rounded-2xl font-black uppercase text-[10px] md:text-xs tracking-widest inline-block text-center">Enter
                            the Feed</a>
                    @else
                        <button @click="openAuth = true; authMode = 'signup'; authRole = 'client'"
                            class="btn-coral w-full py-4 md:py-5 rounded-xl md:rounded-2xl font-black uppercase text-[10px] md:text-xs tracking-widest">Start
                            Discovering</button>
                    @endauth
                </div>
            @endunless
                ...
                <div
                    ...
                    @auth
                        <a href="{{ route('vendor.dashboard') }}"
                            ...
                            my Dashboard</a>
                    @else
                        <button @click="openAuth = true; authMode = 'signup'; authRole = 'restaurateur'"
                            class="btn-coral w-full py-4 md:py-5 rounded-xl md:rounded-2xl font-black uppercase text-[10px] md:text-xs tracking-widest">Grow
                            My Kitchen</button>
                    @endauth
                </div>
            @endunless
    ...
    <footer class="w-full footer-concrete py-12 px-6 md:px-10 mt-20 relative z-20">
        <div
            class="max-w-6xl mx-auto flex items-center gap-3 opacity-50 grayscale hover:grayscale-0 transition-all cursor-default">
            <div
