...
<div class="fixed inset-0 bg-[#090a0c] text-white antialiased flex flex-col md:flex-row overflow-hidden font-sans z-[9999]">
    <style>
        :root { --coral: #FF5A5F; }
        .no-scrollbar::-webkit-scrollbar { display: none; }
        .bg-coral { background-color: var(--coral) !important; }
        .text-coral { color: var(--coral) !important; }
        body { overflow: hidden !important; margin: 0 !important; background-color: #090a0c !important; -webkit-tap-highlight-color: transparent; }
    ...
    <aside class="hidden md:flex w-20 lg:w-64 border-r border-white/5 bg-[#0f1115] flex-col z-50 h-full">
        ...
        <nav class="flex-1 px-3 space-y-2">
            <a href="{{ route('video.feed') }}" class="flex items-center space-x-4 px-4 py-3 rounded-xl text-gray-500 hover:text-white hover:bg-white/5 transition-all group">
            ...
            <a href="{{ route('video.explore') }}" class="flex items-center space-x-4 px-4 py-3 rounded-xl bg-white/5 text-white border border-white/5">
    ...
    <main class="flex-1 h-full overflow-y-auto no-scrollbar relative bg-[#090a0c] pb-32 md:pb-0">
        <div class="max-w-7xl mx-auto p-4 md:p-12">
            ...
            <header class="flex flex-col gap-6 mb-8 md:mb-16">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-gray-500 font-black uppercase text-[8px] md:text-[10px] mt-2 tracking-[0.3em] italic opacity-60">
                            Les pépites à proximité
                        </p>
                    </div>
                </div>
                
                <div class="relative group w-full">
                    <i class="fa-solid fa-magnifying-glass absolute left-5 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-coral transition-colors"></i>
                    <input type="text" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                ...
                <div class="group relative aspect-[9/16] bg-black rounded-[2.5rem] overflow-hidden border border-white/5 md:hover:border-coral/50 transition-all duration-500 shadow-2xl">
                    <video src="{{ asset('storage/' . $clip->video_path) }}" 
                           ...
                           @if(!request()->isMobile()) onmouseover="this.play()" onmouseout="this.pause(); this.currentTime = 0;" @endif
                           muted playsinline loop @if(request()->isMobile()) autoplay @endif>
                    </video>
                    
                    <div class="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-6 flex flex-col justify-end">
                        <div class="md:transform md:translate-y-4 md:group-hover:translate-y-0 transition-transform duration-500">
                            <span class="text-coral font-black text-[9px] uppercase tracking-[0.2em]">#{{ $clip->dish->name ?? 'Gourmet' }}</span>
                            <h3 class="font-black italic text-xl uppercase tracking-tighter text-white mb-4">
                                {{ $clip->restaurant->name ?? 'Vidish Kitchen' }}
                            </h3>
                            
                            <div class="flex items-center justify-between md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-500">
                                <span class="text-[8px] text-gray-400 font-bold uppercase tracking-widest flex items-center">
                                    <i class="fa-solid fa-location-dot mr-1.5 text-coral"></i> 1.2 KM
                                </span>
                                <a href="{{ route('video.feed') }}" class="w-11 h-11 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center hover:bg-coral transition-all border border-white/10 shadow-xl">
            ...
            @else
            <div class="h-[40vh] flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-[3rem] bg-white/[0.01]">
                ...
                <button wire:click="$set('search', '')" class="mt-4 text-coral text-[10px] font-black uppercase tracking-widest hover:underline">Voir tout</button>
            </div>
            @endif
        </div>
    </main>
    ...
    <div class="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-[400px] z-[10000]">
        <nav class="bg-[#16181d]/90 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-2 flex justify-between items-center shadow-[0_20px_50px_rgba(0,0,0,0.5)] safe-bottom">
