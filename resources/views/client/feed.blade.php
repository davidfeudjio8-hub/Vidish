        ...
        <nav class="flex-1 px-3 space-y-2">
            <div class="relative px-2 mb-4">
                <div class="bg-white/5 rounded-xl p-3 flex items-center hover:bg-white/10 transition-all focus-within:ring-1 focus-within:ring-coral">
                    ...
                    <input type="text" x-model="searchQuery" @input="fetchTags()" placeholder="Rechercher #..." 
                ...
                <div x-show="suggestions.length > 0" x-cloak 
                     class="absolute left-0 right-0 mt-2 bg-darkBg border border-white/10 rounded-xl overflow-hidden shadow-2xl z-[100]">
                    <template x-for="tag in suggestions" :key="tag.name">
                        <button @click="searchQuery = '#' + tag.name; suggestions = []" 
            ...
            <a href="{{ route('video.feed') }}" class="flex items-center space-x-4 px-4 py-3 rounded-xl bg-white/5 text-white"><i class="fa-solid fa-house"></i><span class="hidden lg:block text-xs font-bold uppercase">Home</span></a>
            <a href="#" class="flex items-center space-x-4 px-4 py-3 rounded-xl text-gray-500 hover:text-white"><i class="fa-solid fa-compass"></i><span class="hidden lg:block text-xs font-bold uppercase">Explore</span></a>
        </nav>

        <div class="p-4 border-t border-white/5">
            <div class="flex items-center space-x-3 p-2 rounded-2xl hover:bg-white/5 cursor-pointer">
                <div class="w-10 h-10 rounded-full bg-coral flex items-center justify-center font-bold">D</div>
                <div class="hidden lg:block">
                    <p class="text-[10px] font-black uppercase tracking-widest leading-none">{{ Auth::user()->name }}</p>
                </div>
            </div>
        </div>
    </aside>
    ...
    <main class="flex-1 h-screen overflow-y-scroll snap-y no-scrollbar relative">
        ...
        <section class="h-screen w-full snap-start relative flex items-center justify-center overflow-hidden">
            ...
            <div class="h-[92vh] aspect-[9/16] relative bg-black shadow-2xl lg:rounded-[2.5rem] overflow-hidden border border-white/5 z-10">
                <video src="{{ asset('storage/' . $clip->video_path) }}" class="w-full h-full object-cover" loop autoplay muted></video>
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent p-8 flex flex-col justify-end">
                    <div class="flex items-center space-x-4 mb-4">
                        <div class="w-12 h-12 rounded-full border-2 border-coral bg-gray-800 overflow-hidden">
                             </div>
                        <div>
                            <h3 class="font-black italic uppercase tracking-tighter text-lg leading-none">
                                {{ $clip->restaurant->name ?? $clip->dish->restaurant->name ?? 'Vidish Kitchen' }}
                            </h3>
                            <p class="text-xs text-coral font-bold mt-1 tracking-widest">#{{ $clip->dish->name ?? 'Spécialité' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <span class="text-[8px] bg-white/10 px-2 py-1 rounded-md uppercase font-black tracking-widest text-coral">#{{ $tag->name }}</span>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="absolute right-4 bottom-24 flex flex-col space-y-6 z-20">
                    <button class="flex flex-col items-center group">
                        ...
                        <span class="text-[10px] font-black mt-2">1.2k</span>
                    </button>
                    <button @click="commentOpen = true" class="flex flex-col items-center group">
                        ...
                        <span class="text-[10px] font-black mt-2">84</span>
                    </button>
                </div>
            </div>
            ...
            <div x-show="commentOpen" x-cloak 
                ...
                <div class="flex justify-between items-center mb-8">
                    ...
                    <button @click="commentOpen = false" class="text-gray-500 hover:text-white">✕</button>
                </div>
                <div class="flex-1 overflow-y-auto no-scrollbar">
                    <p class="text-gray-500 text-xs font-bold uppercase text-center mt-20 italic">Bientôt disponible...</p>
                </div>
                <div class="mt-6">
                    <input type="text" placeholder="Laisse un avis..." class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:border-coral text-sm">
                </div>
            </div>
        </section>
        @endforeach
